...
const walletState = {
    ...
    tokens: {
        ...
        BTC: { balance: 0, price: 106800 },
        ETH: { balance: 0, price: 3320 },
        BNB: { balance: 0, price: 698 },
...
function updateLivePrices() {
    fetch('https://api.coingecko.com/api/v3/simple/price?ids=pepe,dogecoin,shiba-inu&vs_currencies=usd&include_24hr_change=true')
        .then(res => res.json())
        .then(data => {
            if (data.pepe) {
                const pepePrice = document.getElementById('pepe-price');
                const pepeChange = document.getElementById('pepe-change');
                if (pepePrice) pepePrice.textContent = '$' + data.pepe.usd.toFixed(8);
                if (pepeChange) {
                    const change = data.pepe.usd_24h_change || 0;
                    pepeChange.textContent = (change >= 0 ? '+' : '') + change.toFixed(2) + '%';
                    pepeChange.style.color = change >= 0 ? '#22c55e' : '#ef4444';
                }
            ...
            if (data.dogecoin) {
                ...
                if (dogePrice) dogePrice.textContent = '$' + data.dogecoin.usd.toFixed(4);
                if (dogeChange) {
                    const change = data.dogecoin.usd_24h_change || 0;
                    dogeChange.textContent = (change >= 0 ? '+' : '') + change.toFixed(2) + '%';
                    dogeChange.style.color = change >= 0 ? '#22c55e' : '#ef4444';
                }
                walletState.tokens.DOGE.price = data.dogecoin.usd;
            }
            if (data['shiba-inu']) {
                const shibPrice = document.getElementById('shib-price');
                const shibChange = document.getElementById('shib-change');
                if (shibPrice) shibPrice.textContent = '$' + data['shiba-inu'].usd.toFixed(8);
                if (shibChange) {
                    const change = data['shiba-inu'].usd_24h_change || 0;
                    shibChange.textContent = (change >= 0 ? '+' : '') + change.toFixed(2) + '%';
                    shibChange.style.color = change >= 0 ? '#22c55e' : '#ef4444';
                }
                walletState.tokens.SHIB.price = data['shiba-inu'].usd;
            }
        })
        .catch(() => console.log('Price update failed'));
}
...
function loadWalletData() {
    ...
    // رصيد USDT Flash ثابت 500 دائماً للمستخدمين المسجلين فقط
    if (userData.email && userData.isLoggedIn) {
        ...
        walletState.tokens['USDT-FLASH'].balance = 0;
    }
...
function updateHeroBalance() {
    ...
    if (heroBalance) {
        heroBalance.textContent = totalBalance.toFixed(2);
    }
}
...
if (window.location.pathname.includes('index.html') || window.location.pathname === '/' || window.location.pathname.endsWith('/flash/')) {
    document.addEventListener('DOMContentLoaded', () => {
        const userData = JSON.parse(localStorage.getItem('userData') || '{}');
        let totalBalance = 500; // USDT Flash ثابت 500
        
        if (userData.tokens) {
            Object.keys(userData.tokens).forEach(token => {
                if (token !== 'USDT-FLASH') {
                    const price = token === 'USDT' ? 1.00 : 
                                 token === 'BTC' ? 106800 : 
                                 token === 'ETH' ? 3320 : 
                                 token === 'BNB' ? 698 : 
                                 token === 'PEPE' ? 0.00001845 : 
                                 token === 'DOGE' ? 0.38 : 
                                 token === 'SHIB' ? 0.00002245 : 1.00;
                    totalBalance += (userData.tokens[token] || 0) * price;
                }
            });
        }
